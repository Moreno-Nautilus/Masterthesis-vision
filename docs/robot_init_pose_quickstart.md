...
| `scripts/launch_robots_to_init_pose.sh` | One-terminal wrapper: brings up bringup + MoveIt in the background, waits for both to be ready, then runs `move_to_init_pose.py` in the foreground. **This is the normal entry point** — the two scripts above are its building blocks. |
...
Brings up `hardware.launch.py` + `move_group.launch.py`, waits for MoveGroup
and a plannable state, then plans+executes a single `both_arms_flange`
MoveGroup goal back to the saved pose. Bringup + MoveIt stay running
afterward (`Ctrl+C` to stop them, or keep going with other work — RViz is
already up).
...
CONTROL_MODE=cartesian_impedance scripts/launch_robots_to_init_pose.sh  # see §3
...
## 3. `CONTROL_MODE=cartesian_impedance` (optional, position is still default)
...
obstacle or each other. MoveIt still **plans** the move (same
...
arms are planned+run **one after another**, not simultaneously — each
plan is checked against the other arm's real, live joint state (from
`/joint_states`), so this is still collision-safe, just not simultaneous.
See `move_to_init_pose.py`'s own docstring for why planning per-arm here
(rather than against the `both_arms_flange` composite group) is the
*correct* choice given sequential execution, not a shortcut.

...
class MoveItInsertStages(Node):
    def __init__(self) -> None:
        ...
        self.latched_targets: dict[str, np.ndarray] | None = None
    ...
    def _base_cb(self, msg: PoseStamped) -> None:
        ...
        targets = self._compute_targets(msg)
        if targets is None:
            return

        stamp = self.get_clock().now().to_msg()
        self.pub_hole.publish(T_to_pose(targets["T_base_hole"], "base", stamp))
        self.pub_preinsert.publish(T_to_pose(targets["T_base_preinsert"], "base", stamp))
        self.pub_insert.publish(T_to_pose(targets["T_base_insert"], "base", stamp))
        self.pub_seat.publish(T_to_pose(targets["T_base_seat"], "base", stamp))

        self.pub_preinsert_ee.publish(T_to_pose(targets["T_base_preinsert_ee"], "base", stamp))
    ...
    def _latch_targets_from_latest(self) -> tuple[dict[str, np.ndarray] | None, str]:
        with self._lock:
        ...
        targets = self._compute_targets(latest)
        if targets is None:
            return None, "Failed to compute targets"

        with self._lock:
            self.latched_targets = targets

        return targets, "ok"

    def _get_latched_targets(self) -> tuple[dict[str, np.ndarray] | None, str]:
        with self._lock:
            targets = self.latched_targets

        if targets is None:
            return None, "No latched targets. Call /assembly/preinsert first."
    ...
    def _publish_next_intent(self, targets: dict[str, np.ndarray], next_stage: str) -> None:
        ...
        self.pub_next_stage_name.publish(String(data=next_stage))

        self.get_logger().info(
            f"Next intended stage: {next_stage} | "
    ...
    def _preinsert_cb(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        ...
        targets, msg = self._latch_targets_from_latest()
        if targets is None:
        ...
        ok, msg = self._plan_and_execute_pose(pose_msg, "preinsert")
        response.success = ok
        response.message = msg

        if ok:
            self._publish_next_intent(targets, "insert")
    ...
    def _insert_cb(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        ...
        pose_msg = self._pose_from_target_key(targets, "T_base_insert_ee")
        ok, msg = self._plan_and_execute_pose(pose_msg, "insert")
        response.success = ok
        response.message = msg

        if ok:
            self._publish_next_intent(targets, "seat")
    ...
    def _seat_cb(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        ...
        pose_msg = self._pose_from_target_key(targets, "T_base_seat_ee")
        ok, msg = self._plan_and_execute_pose(pose_msg, "seat")
        response.success = ok
        response.message = msg

        if ok:
            self._publish_next_intent(targets, "retreat")
    ...
    def _retreat_cb(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        ...
        if ok:
            self.pub_next_stage_name.publish(String(data="done"))

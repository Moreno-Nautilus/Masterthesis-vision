...
class DINODebugNode(Node):
    def __init__(
        ...
        self.pub_overlay = self.create_publisher(Image, "/perception/debug/dino_overlay", FAST_QOS)
        self.pub_raw = self.create_publisher(Image, "/perception/debug/dino_raw", FAST_QOS)
    ...
    def _collect_images(self, root: Path) -> List[Path]:
        ...
        for p in sorted(root.rglob("*")):
            if p.is_file() and p.suffix.lower() in exts:
                # skip reference images if image_root is broad
    ...
    def _draw_whole_image_label(self, rgb: np.ndarray, label: str, score: float) -> np.ndarray:
        ...
        cv2.putText(vis, txt, (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2, cv2.LINE_AA)
        return vis
    ...
    def _publish_pair(self, rgb: np.ndarray, overlay: np.ndarray, frame_id: str = "dino_debug") -> None:
        stamp = self.get_clock().now().to_msg()
        self.pub_raw.publish(_rgb_numpy_to_imgmsg(rgb, frame_id=frame_id, stamp=stamp))
        self.pub_overlay.publish(_rgb_numpy_to_imgmsg(overlay, frame_id=frame_id, stamp=stamp))
    ...
    def _tick_images(self) -> None:
        ...
        try:
            ...
            if self.use_sam and self.sam is not None:
                ...
                overlay = self._draw_sam_dino(rgb, masks[:8])
            ...
            self._publish_pair(rgb, overlay, frame_id="image_mode")
            self._index = (self._index + 1) % len(self.image_paths)
    ...
    def _tick_live(self) -> None:
        ...
        try:
            ...
            # for debug, use only first available camera frame
            rgb = None
            frame_id = "live_mode"
            cam_id = None
            for v in views:
                if getattr(v, "rgb", None) is not None:
                    rgb = v.rgb
                    cam_id = v.cam_id
                    frame_id = v.cam_id
                    break
            ...
            if self.use_sam and self.sam is not None:
                ...
                overlay = self._draw_sam_dino(rgb, masks[:8])
                self.get_logger().info(
                    f"[live:{cam_id}] masks={len(masks)} sam={(t1 - t0) * 1000:.1f} ms"
                )
            else:
                t0 = time.perf_counter()
                obj_id, score, scores_by_object = self._classify_single_crop(rgb)
            ...
            self._publish_pair(rgb, overlay, frame_id=frame_id)

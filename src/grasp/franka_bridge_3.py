...
class FrankaGraspBridge(Node):
    ...
    GRASP_MODE_SIDE_AUTO = "side_auto"

    def __init__(self) -> None:
        super().__init__("franka_grasp_bridge")

        # Reentrant callback group so service callbacks can wait on client/action results
        ...
        # Trigger services
        self.srv_base = self.create_service(
            Trigger, "~/go_base", self._go_neutral_cb, callback_group=self.cb_group
        )
        ...
        self.srv_reset = self.create_service(
            Trigger, "~/reset_sequence", self._reset_sequence_cb, callback_group=self.cb_group
        )

        # Dynamic switching services
        self.srv_load_object = self.create_service(
            Trigger, "~/load_object", self._load_object_cb, callback_group=self.cb_group
        )
        self.srv_set_grasp_mode = self.create_service(
            Trigger, "~/set_grasp_mode", self._set_grasp_mode_cb, callback_group=self.cb_group
        )
        self.srv_list_modes = self.create_service(
            Trigger, "~/list_grasp_modes", self._list_grasp_modes_cb, callback_group=self.cb_group
        )
    ...
    def _declare_all_parameters(self) -> None:
        ...
        self.declare_parameter("object_height_m", 0.05)
        ...
        # Side grasp specific
        self.declare_parameter("side_grasp_height_offset_m", 0.0)
        self.declare_parameter("side_pregrasp_standoff_m", 0.10)
        ...
        for key, value in config.items():
            if key in overridable:
                try:
                    ...
                    params_to_set.append(Parameter(key, type_=param.type_, value=value))
                except Exception as e:
                    self.get_logger().warn(f"Skipping config key {key}: {e}")
    ...
    def _log_startup(self) -> None:
        ...
        self.get_logger().info(f"  grasp_mode={self.grasp_mode}")
        self.get_logger().info(
    ...
    def _switch_object(self, object_name: str) -> tuple[bool, str]:
        ...
        try:
            ...
            self.get_logger().warn(f"Failed to destroy old subscription cleanly: {e}")
    ...
    def _compute_pregrasp_and_grasp(self, msg: PoseStamped) -> tuple[TargetPose, TargetPose]:
        ...
        if self.grasp_mode == self.GRASP_MODE_SIDE_X:
            return self._compute_side_grasp(x_c, y_c, z_c, axis="x")
        if self.grasp_mode == self.GRASP_MODE_SIDE_Y:
            return self._compute_side_grasp(x_c, y_c, z_c, axis="y")
        if self.grasp_mode == self.GRASP_MODE_SIDE_AUTO:
    ...
    def _compute_side_grasp_auto(
        ...
        self.get_logger().info(f"Auto side grasp: scores={scores}, selected={best_axis}")
    ...
    # ------------------------------------------------------------------
    # Future waiting helpers
    # ------------------------------------------------------------------

    def _wait_for_future(self, future, timeout_s: float) -> bool:
        """
        Wait for a future while the MultiThreadedExecutor keeps spinning
        elsewhere. No spin_once here.
        """
    ...
    # ------------------------------------------------------------------
    # Blocking calls that now work with the reentrant + multithreaded setup
    # ------------------------------------------------------------------
    ...
    def _send_pose_blocking(self, target: TargetPose) -> tuple[bool, str]:
        ...
        if not self._wait_for_future(future, self.set_pose_timeout_s):
            return False, f"set_pose timed out after {self.set_pose_timeout_s:.1f}s"
    ...
    def _close_gripper_blocking(self) -> tuple[bool, str]:
        ...
        if not self._wait_for_future(result_future, self.gripper_timeout_s):
            return False, f"Gripper action timed out after {self.gripper_timeout_s:.1f}s"

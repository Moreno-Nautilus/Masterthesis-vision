...
class FoundationPoseTrackerNode(Node):
    ...
    def _draw_sam_overlay(self, cam_id: str, rgb: np.ndarray, masks: list[SAMMaskCandidate]) -> np.ndarray:
        ...
        vis = draw_roi_rectangle(vis, x_min, x_max, y_min, y_max, label=f"ROI {cam_id}")
    ...
    def _initialize_objects(
        ...
        for i, sel in enumerate(selections):
            ...
            state.last_logged_T_base = None #self._safe_to_base_pose(cam_id, T).copy()
            state.last_logged_convention = None # track_pose_convention
    ...
    def _track_objects(
        ...
        for i, state in enumerate(states):
            if state.T_object_camera is None or state.fp_tracker is None:
                state.lost_count += 1
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cam1-sam-min-mask-area", type=int, default=600)
    p.add_argument("--cam1-sam-min-bbox-side-px", type=int, default=10)
    p.add_argument("--cam1-sam-max-mask-area-ratio", type=float, default=0.10)
    p.add_argument("--cam1-sam-max-bbox-area-ratio", type=float, default=0.12)
    p.add_argument("--cam1-sam-border-px", type=int, default=6)
    p.add_argument("--cam1-sam-max-border-fraction", type=float, default=0.00)
    p.add_argument("--cam1-roi-left-px", type=int, default=250)
    p.add_argument("--cam1-roi-right-px", type=int, default=250)
    p.add_argument("--cam1-roi-top-px", type=int, default=250)
    p.add_argument("--cam1-roi-bottom-px", type=int, default=10)

    # Camera 2 SAM/filter params = current values
    p.add_argument("--cam2-sam-min-mask-area", type=int, default=600)
    p.add_argument("--cam2-sam-min-bbox-side-px", type=int, default=10)
    p.add_argument("--cam2-sam-max-mask-area-ratio", type=float, default=0.10)
    p.add_argument("--cam2-sam-max-bbox-area-ratio", type=float, default=0.12)
    ...
    p.add_argument("--cam2-roi-top-px", type=int, default=250)

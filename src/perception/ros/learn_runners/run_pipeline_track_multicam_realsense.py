"""Multi-camera pose pipeline node for the 1-ZED + 2-RealSense trio.

Copy of run_pipeline_track_multicam.py with only the camera registry,
extrinsics loading, and grabber wiring changed to support a heterogeneous
camera set: one static tripod-mounted ZED (zed2i_1) plus two end-effector
-mounted RealSense cameras whose extrinsics are recomputed every frame from
a live flange pose (see multicam_grabber_realsense.py). The detection /
fusion / tracking pipeline body (FoundationPoseTrackerNode and below) is
...
def main() -> None:
    ...
    # zed2i_1 -> real camera-to-base extrinsic (static).
    # realsense_1 / realsense_2 -> camera-to-FLANGE offset (static mount),
    ...
    # Re-express every base-frame transform relative to the dual-arm bringup's
    # base_link (the midpoint between robot_a and robot_b, oriented like
    # robot_a) instead of robot_a's own frame, so both the static ZED extrinsic
    # and every live flange pose (each shifted per-arm via robot_bases in
    # MultiCamGrabberRealsense) land in base_link.
    T_robotA_baseLink = get_dual_arm_base_link()
    T_baseLink_robotA = T_robotA_baseLink.inverse()
    T_robotA_activeRobot = T_baseLink_robotA.compose(T_robotA_activeRobot)
    robot_bases = {name: T_baseLink_robotA.compose(T) for name, T in robot_bases.items()}
    print(
    ...
    # passthrough for zed2i_1, live T_base_flange @ T_flange_cam for the two
    # RealSense cams) — pass the same object through so FoundationPoseTrackerNode
    # and multicam_fusion transparently get live extrinsics via T_map[cam_id].
    node = FoundationPoseTrackerNode(
        args=args, grabber=grabber, T_base_cam_map=grabber.T_base_cam_map
    )

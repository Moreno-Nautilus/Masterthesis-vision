...
# Pull the 3x3 intrinsics out of a CameraInfo message.
def _K_from_camerainfo(msg: CameraInfo) -> np.ndarray:
    return np.array(msg.k, dtype=float).reshape(3, 3)


# Convert raw depth to meters depending on its encoding (float vs millimetre int).
...
def _img_to_numpy_depth(msg: Image) -> np.ndarray:
    ...
    # Fast path when rows are tightly packed; else copy row by row stripping padding.
    if step_elems == w:
...
def _img_to_numpy_rgb(msg: Image) -> np.ndarray:
    ...
    # Pick channel count from the encoding.
    if enc in ("rgb8", "bgr8"):
        channels = 3
    ...
    # Reshape directly when packed; else strip per-row padding.
    ...
    # Drop alpha, and flip BGR to RGB so downstream is always RGB.
    if channels == 4:
        img = img[:, :, :3]
...
# ROS node that buffers each camera's latest RGB/depth/intrinsics and serves time-synced View sets.
class MultiCamGrabber(Node):
    ...
    def ready(self) -> bool:
        # True only once every camera has both images and both intrinsics buffered.
    ...
    def get_latest_views(self) -> Optional[list[View]]:
        ...
        for c in self.cameras:
            ...
            # Per camera, also require its rgb and depth stamps to be close.
            ...
            # Decode depth to meters; backprojection uses the depth intrinsics.
            ...
            if rgb is not None:
                ...
                # Cheap frozen-feed detector: hash a subsampled frame and count repeats.

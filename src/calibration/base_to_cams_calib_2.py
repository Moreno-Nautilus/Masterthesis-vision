...
SYNC_SLOP_S = 0.05              # cam1 RGB vs cam2 RGB
RGB_INFO_MAX_DT_S = 0.10        # per camera: RGB vs CameraInfo
MAX_WAIT_S = 60.0               # max time waiting for valid pair
PRINT_EVERY_S = 5.0

# multi-sample robustness
NUM_SAMPLES = 8                 # number of accepted stereo captures to average
MIN_SAMPLES_TO_SOLVE = 5
INTER_SAMPLE_MIN_DT_S = 0.4     # avoid taking near-identical frames
MAX_REPROJ_ERR_PX = 1.2         # reject single sample if too high
...
class CheckerboardBaseCalib(Node):
    ...
    def ready(self) -> bool:
        return self.cam1.has_fresh_pair() and self.cam2.has_fresh_pair()

    def get_synced_pair(self) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float]]:
        if not self.ready():
            return None

        dt_cam1 = abs(self.cam1.img_t - self.cam1.info_t)
        dt_cam2 = abs(self.cam2.img_t - self.cam2.info_t)
        if dt_cam1 > RGB_INFO_MAX_DT_S or dt_cam2 > RGB_INFO_MAX_DT_S:
            return None

        cross_dt = abs(self.cam1.img_t - self.cam2.img_t)
        if cross_dt > SYNC_SLOP_S:
            return None

        img1 = _img_to_numpy_color(self.cam1.img_msg)
        img2 = _img_to_numpy_color(self.cam2.img_msg)
        return img1, img2, self.cam1.K.copy(), self.cam2.K.copy(), dt_cam1, dt_cam2
...
def main() -> None:
    ...
    print("")
    print("=== Robust stereo-base calibration ===")
    print(f"Need {NUM_SAMPLES} accepted stereo samples")
    print("Show the checkerboard to BOTH cameras and hold it steady.")
    ...
    while len(accepted) < NUM_SAMPLES:
        ...
        if now - t_start > MAX_WAIT_S:
            raise RuntimeError("Timed out waiting for enough valid stereo captures.")
        ...
        if reproj1 > MAX_REPROJ_ERR_PX or reproj2 > MAX_REPROJ_ERR_PX:
            print(
                f"[reject] reproj too high: cam1={reproj1:.3f}px cam2={reproj2:.3f}px "
        ...
        print(
            ...
            f"rgb-info dt: cam1={dt1:.4f}s cam2={dt2:.4f}s | "
            f"reproj: cam1={reproj1:.3f}px cam2={reproj2:.3f}px"
        )
    ...
    if tstd1 > MAX_FINAL_TRANSLATION_STD_M or tstd2 > MAX_FINAL_TRANSLATION_STD_M:
        raise RuntimeError(
            f"Calibration rejected: translation spread too high "
            f"(cam1={tstd1:.6f}m, cam2={tstd2:.6f}m)"
        )

    if rstd1 > MAX_FINAL_ROTATION_STD_DEG or rstd2 > MAX_FINAL_ROTATION_STD_DEG:
        raise RuntimeError(
            f"Calibration rejected: rotation spread too high "
            f"(cam1={rstd1:.6f}deg, cam2={rstd2:.6f}deg)"
        )

...
#   scripts/launch_pipeline.sh fast-track --enable-debug-frames
#
...
FAST_TRACK_ARGS=(
    "${TRACK_BASE_ARGS[@]}"
    --log-track-poses
    --track-pose-log-path outputs/logs/live_fast_track_q.csv
)
...
ACCURATE_TRACK_ARGS=(
    ...
    --log-init-poses
    --log-track-poses
    --track-pose-log-path outputs/logs/live_accurate_track_q.csv
)
...
case "${1:-}" in
    ...
    baseline|init-only|init_only)
        MODE_NAME="init-only"
        MODE_LOG="outputs/logs/multicam_init_final_baseline.log"
        shift
        set -- "${INIT_ONLY_ARGS[@]}" "$@"
        ;;
    fast-track|fast_track|fast)
        MODE_NAME="fast-track"
        MODE_LOG="outputs/logs/live_fast_track.log"
        shift
        set -- "${FAST_TRACK_ARGS[@]}" "$@"
        ;;
    accurate-track|accurate_track|accurate)
        MODE_NAME="accurate-track"
        MODE_LOG="outputs/logs/live_accurate_track.log"
        shift
